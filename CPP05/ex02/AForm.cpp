...
AForm::~AForm() {
    std::cout << BLACK << "Destructor called for " << _name << "'s AForm (base)" RESET << std::endl;
}
...
void AForm::beSigned(Bureaucrat &brc) {
    if (brc.getGrade() <= this->getSignGrade() && this->getSigned() == 0)
        this->_signed = 1;
    else if (brc.getGrade() <= this->getSignGrade() && this->getSigned() == 1)
...
std::ostream& operator<<(std::ostream& os, const AForm& f) {
    ...
    os << "| Signed (True = 1 | False = 0): " << f.getSigned() << std::endl;
    os << "| Minimum Grade to Sign: " << f.getSignGrade() << std::endl;
    os << "| Minimum Grade to Execute: " << f.getExecuteGrade() << std::endl;
    return (os);
}
